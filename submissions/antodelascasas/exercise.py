...
def numpy_order_revenues(quantities: np.ndarray, unit_prices: np.ndarray) -> np.ndarray:
    ...
    """
    return np.asarray(quantities) * np.asarray(unit_prices)
...
def pandas_region_share(sales: pd.DataFrame) -> pd.DataFrame:
    ...
    grand_total = pd.DataFrame(
        {"_key": [0], "total_revenue": [by_region["revenue"].sum()]}
    )
    merged = by_region.assign(_key=0).merge(grand_total, on="_key")
    return merged.assign(share=merged["revenue"] / merged["total_revenue"])[
